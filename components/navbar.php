...
<nav class="nav">
    <div class="nav__container">
        <a href=<?= $isLoggedIn ? BASE_URL . "/pages/control.php" : BASE_URL . "/index.php" ?> class="nav__logo">
        ...
        <div class="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
            <a href="?logout" class="text-orange bg-brand hover:bg-brand-strong box-border border border-transparent focus:ring-4 focus:ring-brand-medium shadow-xs font-bold leading-5 rounded-base text-xl px-3 py-2 focus:outline-none">Logout</a>
        ...
        <?php else: ?>
        <div class="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
            <a href="<?= BASE_URL?>/login.php" class="nav__button">Login</a>
            <button data-collapse-toggle="navbar-sticky" type="button" class="inline-flex items-center p-2 w-10 h-10 justify-center text-sm md:hidden focus:outline-none  text-orange" aria-controls="navbar-sticky" aria-expanded="false">
                <span class="sr-only">Open main menu</span>
                <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M5 7h14M5 12h14M5 17h14"/></svg>
            </button>
        </div>
        <div class="absolute bg-transparent md:min-h-fit min-h-[60vh] left-0 top-[9%] w-full flex items-center px-5">
            <ul class="flex md:flow-row flex-col md:items-center md:gap-[4vw] gap-8">
